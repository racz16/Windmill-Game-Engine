...
namespace wm {
	...
	private:
		std::vector<ptr_view<log_writer>> log_writers {};
	public:
		void add_log_writer(const ptr_view<log_writer> writer) override;
		size_t get_log_writer_count() const override;
		ptr_view<log_writer> get_log_writer(const int32_t index) const override;
		void remove_log_writer(const ptr_view<log_writer> log_writer) override;
