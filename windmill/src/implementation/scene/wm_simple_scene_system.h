...
namespace wm {
	...
	protected:
		...
		ptr<transform> get_transform_impl(const ptr_view<node> node) const;
		ptr<node> get_node_by_component_impl(const ptr_view<component> component) const override;
		ptr<component> get_component_impl(const ptr_view<node> node, const key<component>& key) const override;
		std::vector<ptr<component>> get_components(const ptr_view<node> node) const;
		std::vector<ptr<component>> get_components_impl(const ptr_view<node> node, const key<component>& key) const override;
	...
	public:
		...
		bool is_descendant_of(const ptr_view<node> descendant, const ptr_view<node> ancestor) const override;
		ptr<node> get_parent(const ptr_view<node> node) const override;
		std::vector<ptr<node>> get_children(const ptr_view<node> node) const override;
		ptr<node> get_node_by_transform(const ptr_view<transform> transform) const override;
		std::vector<ptr<node>> get_nodes() const;
		...
		ptr_view<transform> get_transform(const ptr_view<node> node) const override;

		bool contains_any_tags(const ptr_view<node> node) const override;
		bool contains_tag(const ptr_view<node> node, const tag& tag) const override;
		std::vector<tag> get_tags(const ptr_view<node> node) const override;
		...
		bool contains_node_by_tag(const ptr_view<node> node, const tag& tag) const override;
		std::vector<ptr<node>> get_nodes_by_tag(const tag& tag) const override;
		std::vector<ptr<node>> get_nodes_by_tags(std::function<bool(const std::vector<tag>& tags)> func) const override;
