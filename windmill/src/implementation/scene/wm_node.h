...
namespace wm {
	...
	protected:
		std::vector<ptr<component>> get_components_impl(const key<component>& key) const override;
	...
	public:
		...
		bool is_ancestor_of(const ptr_view<node> node) const override;
		bool is_descendant_of(const ptr_view<node> node) const override;
		ptr<node> get_root() const override;
		ptr<node> get_parent() const override;
		std::vector<ptr<node>> get_children() const override;
		...
		std::vector<tag> get_tags() const override;
