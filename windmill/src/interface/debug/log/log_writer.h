...
namespace wm {
	...
	public:
		static std::shared_ptr<log_writer> create_console_log_writer(const log_level max_log_level = log_level::Debug);
		static std::shared_ptr<log_writer> create_file_log_writer(const log_level max_log_level = log_level::Debug, const std::string& path = "log");
		virtual log_level get_max_log_level() = 0;
		virtual void set_max_log_level(const log_level level) = 0;
		virtual bool is_force_flush() = 0;
