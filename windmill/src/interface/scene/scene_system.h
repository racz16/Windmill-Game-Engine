...
namespace wm {
	...
	protected:
		...
		//component
		virtual ptr<node> get_node_by_component_impl(const ptr_view<wm::component> component) const = 0;
		virtual ptr<component> get_component_impl(const ptr_view<node> node, const key<component>& key) const = 0;
		virtual std::vector<ptr<component>> get_components_impl(const ptr_view<node> node, const key<component>& key) const = 0;
	...
	public:
		...
		//parent-child
		virtual bool is_descendant_of(const ptr_view<node> descendant, const ptr_view<node> ancestor) const = 0;
		virtual ptr<node> get_parent(const ptr_view<node> node) const = 0;
		virtual std::vector<ptr<node>> get_children(const ptr_view<node> node) const = 0;
		...
		//transform
		virtual ptr<node> get_node_by_transform(const ptr_view<transform> transform) const = 0;
		virtual ptr_view<transform> get_transform(const ptr_view<node> node) const = 0;

		//tags
		virtual bool contains_any_tags(const ptr_view<node> node) const = 0;
		virtual bool contains_tag(const ptr_view<node> node, const tag& tag) const = 0;
		virtual std::vector<tag> get_tags(const ptr_view<node> node) const = 0;
		...
		virtual bool contains_node_by_tag(const ptr_view<node> node, const tag& tag) const = 0;
		virtual std::vector<ptr<node>> get_nodes_by_tag(const tag& tag) const = 0;
		virtual std::vector<ptr<node>> get_nodes_by_tags(std::function<bool(const std::vector<tag>& tags)> func) const = 0;
		...
		//components
		template<class T>
		ptr<node> get_node_by_component(const ptr_view<T> component) const {
			return get_node_by_component_impl(component.template convert<wm::component>());
		}

		template<class T>
		std::vector<ptr<T>> get_components(const ptr_view<node> node, const key<T>& key) const {
		...
		template<class T>
		ptr<T> get_component(const ptr_view<node> node, const key<T>& key) const {
			wm::key<wm::component> new_key{key.get_name()};
