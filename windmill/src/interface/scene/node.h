...
namespace wm {
	...
	protected:
		...
		virtual ptr<node> get_ptr() const;
		virtual void initialize(const int32_t id);
		virtual void initialize_node(const ptr<transform> transform);

		virtual std::vector<ptr<component>> get_components_impl(const key<component>& key) const = 0;
	...
	public:
		...
		virtual bool is_ancestor_of(const ptr_view<node> node) const = 0;
		virtual bool is_descendant_of(const ptr_view<node> node) const = 0;
		virtual ptr<node> get_root() const = 0;
		virtual ptr<node> get_parent() const = 0;
		virtual std::vector<ptr<node>> get_children() const = 0;
		...
		virtual std::vector<tag> get_tags() const = 0;

		template<class T>
		std::vector<ptr<T>> get_components(const key<T>& key) const {
